    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#111118;border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:36px 32px;color:#e8e8e0">
        ...
        <tr><td style="padding-top:28px">
          <p style="margin:0 0 16px;font-size:1rem;font-weight:700;color:#e8e8e0">Olá, {nome}!</p>
          <p style="margin:0 0 16px;color:#888;font-size:.9rem;line-height:1.65">
            Sua assinatura ContaIA vence em <strong style="color:#d4a843">{dias} dias</strong> ({data_venc}).
            Para manter o acesso sem interrupção, renove seu plano.
          </p>
          <table cellpadding="0" cellspacing="0" style="margin:20px 0;width:100%">
            {features}
          </table>
...
def send_subscription_expiry_warning(user) -> bool:
    ...
    if not current_app.config.get("MAIL_USERNAME"):
        log.debug("MAIL_USERNAME não configurado — e-mail D-7 assinatura ignorado")
        return False
    try:
        dias = user.subscription_days_remaining or 7
        data_venc = user.subscription_end.strftime("%d/%m/%Y") if user.subscription_end else "—"
        url_planos = url_for("main.planos", _external=True)
        features_html = "".join(_FEATURE_ROW.format(text=f) for f in _FEATURES)
        html_body = _SUBSCRIPTION_WARNING_HTML.format(
            nome=user.first_name,
            dias=dias,
            data_venc=data_venc,
        ...
        msg = Message(
            subject=f"Sua assinatura ContaIA vence em {dias} dias",
            recipients=[user.email],
            html=html_body,
        )
        mail.send(msg)
        log.info("E-mail D-7 assinatura enviado para %s", user.email)
        return True
    except Exception as exc:
        log.error("Erro ao enviar e-mail D-7 assinatura para %s: %s", user.email, exc)
        return False
